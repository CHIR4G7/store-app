...
Deno.serve(async (req) => {
  ...
  if (!body.newStatus || !allowedStatuses.has(body.newStatus)) {
    return json({ success: false, message: "A valid newStatus is required" }, 400);
  }
  ...
  if (!profile.is_active || (profile.role !== "worker" && profile.role !== "admin")) {
    return json({ success: false, message: "Only active workers or admins can update order status" }, 403);
  }
